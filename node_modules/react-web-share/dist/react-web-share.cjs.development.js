'use strict';

Object.defineProperty(exports, '__esModule', { value: true });

function _interopDefault (ex) { return (ex && (typeof ex === 'object') && 'default' in ex) ? ex['default'] : ex; }

var React = require('react');
var React__default = _interopDefault(React);
var reactDom = require('react-dom');

function _extends() {
  _extends = Object.assign || function (target) {
    for (var i = 1; i < arguments.length; i++) {
      var source = arguments[i];

      for (var key in source) {
        if (Object.prototype.hasOwnProperty.call(source, key)) {
          target[key] = source[key];
        }
      }
    }

    return target;
  };

  return _extends.apply(this, arguments);
}

var BackdropContainerStyle = {
  position: "fixed",
  left: 0,
  top: 0,
  width: "100%",
  height: "100%",
  background: "rgba(0, 0, 0, 0.4)",
  display: "flex",
  flexDirection: "column",
  alignItems: "center",
  justifyContent: "flex-end",
  zIndex: 1400
};
var inEffect = ".web-share-fade{animation-name:simpleFade;animation-duration:.2s;animation-fill-mode:both}@keyframes simpleFade{0%{opacity:0}100%{opacity:1}}.web-share-fade-in-up{animation-name:fadeInUp;animation-duration:.45s;animation-fill-mode:both}@keyframes fadeInUp{0%{opacity:0;transform:translateY(20px)}100%{opacity:1;transform:translateY(0)}}";
function Backdrop(_ref) {
  var children = _ref.children,
      onClose = _ref.onClose;

  var handleOnClose = function handleOnClose(e) {
    if (e.target === e.currentTarget) {
      onClose(e);
    }
  };

  return React__default.createElement("div", {
    onClick: handleOnClose,
    className: "web-share-fade",
    style: BackdropContainerStyle
  }, React__default.createElement("style", {
    children: inEffect
  }), children);
}

var iconList = {
  facebook: {
    path: /*#__PURE__*/React__default.createElement("path", {
      d: "M23.998 12c0-6.628-5.372-12-11.999-12C5.372 0 0 5.372 0 12c0 5.988 4.388 10.952 10.124 11.852v-8.384H7.078v-3.469h3.046V9.356c0-3.008 1.792-4.669 4.532-4.669 1.313 0 2.686.234 2.686.234v2.953H15.83c-1.49 0-1.955.925-1.955 1.874V12h3.328l-.532 3.469h-2.796v8.384c5.736-.9 10.124-5.864 10.124-11.853z"
    }),
    color: "#0076FB",
    e: function e(l) {
      return window.open("https://www.facebook.com/sharer/sharer.php?u=" + l);
    }
  },
  twitter: {
    path: /*#__PURE__*/React__default.createElement("path", {
      d: "M24 4.6a10 10 0 01-2.9.7 5 5 0 002.2-2.7c-1 .6-2 1-3.1 1.2a5 5 0 00-8.4 4.5A14 14 0 011.6 3.2 4.8 4.8 0 001 5.6a5 5 0 002.2 4.1 4.9 4.9 0 01-2.3-.6A5 5 0 005 14a5 5 0 01-2.2 0 5 5 0 004.6 3.5 9.9 9.9 0 01-6.1 2.1H0a14 14 0 007.6 2.2c9 0 14-7.5 14-14V7A10 10 0 0024 4.6z"
    }),
    color: "#1DA1F2",
    e: function e(l, t) {
      return window.open("http://twitter.com/share?text=" + t + "&url=" + l);
    }
  },
  whatsapp: {
    path: /*#__PURE__*/React__default.createElement("path", {
      d: "M17.47 14.38c-.3-.15-1.76-.87-2.03-.97-.27-.1-.47-.14-.67.15-.2.3-.76.97-.94 1.17-.17.2-.34.22-.64.07s-1.26-.46-2.4-1.47a8.95 8.95 0 01-1.64-2.06c-.18-.3-.02-.46.13-.6.13-.14.3-.35.44-.53.15-.17.2-.3.3-.5.1-.2.05-.36-.03-.51l-.91-2.21c-.24-.58-.49-.5-.67-.51l-.57-.01c-.2 0-.52.07-.8.37-.26.3-1.03 1.02-1.03 2.48s1.06 2.88 1.21 3.08c.15.2 2.1 3.2 5.08 4.48.7.3 1.26.5 1.7.63.7.22 1.35.2 1.86.11.57-.08 1.76-.71 2-1.4.26-.7.26-1.3.18-1.42-.07-.13-.27-.2-.57-.35m-5.42 7.4A9.87 9.87 0 017 20.41l-.36-.22-3.74.99 1-3.65-.23-.38a9.86 9.86 0 01-1.51-5.26 9.9 9.9 0 0116.87-6.98 9.82 9.82 0 012.9 7 9.9 9.9 0 01-9.89 9.88m8.41-18.3A11.81 11.81 0 0012.05 0a11.91 11.91 0 00-10.3 17.84L.05 24l6.31-1.65a11.88 11.88 0 005.68 1.44h.01c6.55 0 11.89-5.33 11.9-11.89a11.82 11.82 0 00-3.49-8.41z"
    }),
    color: "#25D366",
    e: function e(l, t) {
      return window.open("https://api.whatsapp.com/send?text=" + t + " " + l);
    }
  },
  reddit: {
    path: /*#__PURE__*/React__default.createElement("path", {
      d: "M12 0A12 12 0 000 12a12 12 0 0012 12 12 12 0 0012-12A12 12 0 0012 0zm5.01 4.744c.688 0 1.25.561 1.25 1.249a1.25 1.25 0 01-2.498.056l-2.597-.547-.8 3.747c1.824.07 3.48.632 4.674 1.488.308-.309.73-.491 1.207-.491.968 0 1.754.786 1.754 1.754 0 .716-.435 1.333-1.01 1.614a3.111 3.111 0 01.042.52c0 2.694-3.13 4.87-7.004 4.87-3.874 0-7.004-2.176-7.004-4.87 0-.183.015-.366.043-.534A1.748 1.748 0 014.028 12c0-.968.786-1.754 1.754-1.754.463 0 .898.196 1.207.49 1.207-.883 2.878-1.43 4.744-1.487l.885-4.182a.342.342 0 01.14-.197.35.35 0 01.238-.042l2.906.617a1.214 1.214 0 011.108-.701zM9.25 12C8.561 12 8 12.562 8 13.25c0 .687.561 1.248 1.25 1.248.687 0 1.248-.561 1.248-1.249 0-.688-.561-1.249-1.249-1.249zm5.5 0c-.687 0-1.248.561-1.248 1.25 0 .687.561 1.248 1.249 1.248.688 0 1.249-.561 1.249-1.249 0-.687-.562-1.249-1.25-1.249zm-5.466 3.99a.327.327 0 00-.231.094.33.33 0 000 .463c.842.842 2.484.913 2.961.913.477 0 2.105-.056 2.961-.913a.361.361 0 00.029-.463.33.33 0 00-.464 0c-.547.533-1.684.73-2.512.73-.828 0-1.979-.196-2.512-.73a.326.326 0 00-.232-.095z"
    }),
    color: "#FF4500",
    e: function e(l, t) {
      return window.open("http://www.reddit.com/submit?url=" + l + "&title=" + t);
    }
  },
  telegram: {
    path: /*#__PURE__*/React__default.createElement("path", {
      d: "M23.91 3.79L20.3 20.84c-.25 1.21-.98 1.5-2 .94l-5.5-4.07-2.66 2.57c-.3.3-.55.56-1.1.56-.72 0-.6-.27-.84-.95L6.3 13.7l-5.45-1.7c-1.18-.35-1.19-1.16.26-1.75l21.26-8.2c.97-.43 1.9.24 1.53 1.73z"
    }),
    color: "#0088CC",
    e: function e(l, t) {
      return window.open("https://telegram.me/share/msg?url=" + l + "&text=" + t);
    }
  },
  linkedin: {
    path: /*#__PURE__*/React__default.createElement("path", {
      d: "M6.5 21.5h-5v-13h5v13zM4 6.5C2.5 6.5 1.5 5.3 1.5 4s1-2.4 2.5-2.4c1.6 0 2.5 1 2.6 2.5 0 1.4-1 2.5-2.6 2.5zm11.5 6c-1 0-2 1-2 2v7h-5v-13h5V10s1.6-1.5 4-1.5c3 0 5 2.2 5 6.3v6.7h-5v-7c0-1-1-2-2-2z"
    }),
    color: "#0073b1",
    e: function e(l, t, ti) {
      return window.open("https://www.linkedin.com/shareArticle?mini=true&url=" + l + "&title=" + ti + "&summary=" + t);
    }
  },
  mail: {
    path: /*#__PURE__*/React__default.createElement("path", {
      d: "M20 4H4c-1.1 0-1.99.9-1.99 2L2 18c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V6c0-1.1-.9-2-2-2zm0 4l-8 5-8-5V6l8 5 8-5v2z"
    }),
    color: "#E53E3E",
    e: function e(l, t) {
      return window.open("mailto:?body=" + l + "&subject=" + t);
    }
  },
  copy: {
    path: /*#__PURE__*/React__default.createElement("path", {
      d: "M16 1H4c-1.1 0-2 .9-2 2v14h2V3h12V1zm3 4H8c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h11c1.1 0 2-.9 2-2V7c0-1.1-.9-2-2-2zm0 16H8V7h11v14z"
    }),
    color: "#718096",
    e: function e(l) {
      return navigator.clipboard.writeText(l);
    }
  }
};

var PortalComponent = function PortalComponent(_ref) {
  var children = _ref.children;
  var el = document.createElement("div");
  React.useEffect(function () {
    document.body.appendChild(el);
    return function () {
      document.body.removeChild(el);
    };
  }, [el]);
  return reactDom.createPortal(children, el);
};

var CloseStyle = {
  background: "#edf2f7",
  cursor: "pointer",
  padding: "0.75rem",
  display: "block",
  width: "100%",
  border: 0,
  fontSize: "1rem"
};
function CloseButton(_ref) {
  var onClose = _ref.onClose;
  return React__default.createElement("button", {
    style: CloseStyle,
    "aria-label": "Close",
    type: "button",
    onClick: onClose
  }, "Close");
}

var ShareHeader = {
  padding: "1rem 1.25rem",
  fontSize: "1.25rem",
  fontWeight: 600,
  paddingBottom: 0
};
function Header(_ref) {
  var title = _ref.title;
  return React__default.createElement("header", {
    style: ShareHeader
  }, title);
}

var SocialIconStyle = {
  width: "100%",
  height: "auto",
  cursor: "pointer",
  border: 0,
  background: "#1A78F6",
  padding: "0.75rem",
  borderRadius: "0.5rem",
  fontSize: 0
};
function Icon(_ref) {
  var name = _ref.name,
      data = _ref.data,
      onClose = _ref.onClose;
  var _iconList$name = iconList[name],
      path = _iconList$name.path,
      _iconList$name$viewBo = _iconList$name.viewBox,
      viewBox = _iconList$name$viewBo === void 0 ? "0 0 24 24" : _iconList$name$viewBo,
      color = _iconList$name.color,
      e = _iconList$name.e;
  return React__default.createElement("button", {
    onClick: function onClick() {
      return e(data.url, data.text, data.title) && onClose();
    },
    "aria-label": name,
    style: _extends({}, SocialIconStyle, {
      background: color
    })
  }, React__default.createElement("svg", {
    xmlns: "http://www.w3.org/2000/svg",
    fill: "white",
    viewBox: viewBox
  }, path));
}

var IconsContainer = {
  maxWidth: "24rem",
  width: "90%",
  backgroundColor: "white",
  borderRadius: "0.5rem",
  borderBottomLeftRadius: 0,
  borderBottomRightRadius: 0
};
var IconsLisStyle = {
  padding: "1.25rem",
  display: "grid",
  gridTemplateColumns: "repeat(4, 1fr)",
  gridGap: "1.25rem"
};
function SocialIcons(_ref) {
  var onClose = _ref.onClose,
      sites = _ref.sites,
      data = _ref.data;
  return React__default.createElement("section", {
    role: "dialog",
    "aria-modal": "true",
    className: "web-share-fade-in-up",
    style: IconsContainer
  }, React__default.createElement(Header, {
    title: data.title
  }), React__default.createElement("div", {
    style: IconsLisStyle
  }, sites.map(function (name) {
    return React__default.createElement(Icon, {
      name: name,
      key: name,
      data: data,
      onClose: onClose
    });
  })), React__default.createElement(CloseButton, {
    onClose: onClose
  }));
}

function useDisclosure(isOpenDefault) {
  if (isOpenDefault === void 0) {
    isOpenDefault = false;
  }

  var _useState = React.useState(isOpenDefault),
      isOpen = _useState[0],
      setIsOpen = _useState[1];

  var onOpen = React.useCallback(function () {
    return setIsOpen(true);
  }, []);
  var onClose = React.useCallback(function () {
    return setIsOpen(false);
  }, []);
  var onToggle = React.useCallback(function () {
    return setIsOpen(function (state) {
      return !state;
    });
  }, []);
  return {
    isOpen: isOpen,
    onOpen: onOpen,
    onClose: onClose,
    onToggle: onToggle
  };
}

var RWebShare = function RWebShare(_ref) {
  var children = _ref.children,
      data = _ref.data,
      _ref$sites = _ref.sites,
      sites = _ref$sites === void 0 ? Object.keys(iconList) : _ref$sites;

  var _useDisclosure = useDisclosure(),
      onOpen = _useDisclosure.onOpen,
      onClose = _useDisclosure.onClose,
      isOpen = _useDisclosure.isOpen;

  var shareData = _extends({
    title: "share",
    text: "",
    url: typeof window !== "undefined" ? window.location.href : undefined
  }, data);

  var handleOnClick = function handleOnClick() {
    if (window.navigator.share) {
      window.navigator.share(shareData)["catch"](console.error);
    } else {
      onOpen();
    }
  };

  return React__default.createElement(React__default.Fragment, null, React.cloneElement(children, _extends({}, children === null || children === void 0 ? void 0 : children.props, {
    onClick: handleOnClick
  })), isOpen && React__default.createElement(PortalComponent, null, React__default.createElement(Backdrop, {
    onClose: onClose
  }, React__default.createElement(SocialIcons, {
    onClose: onClose,
    sites: sites,
    data: shareData
  }))));
};

exports.RWebShare = RWebShare;
//# sourceMappingURL=react-web-share.cjs.development.js.map
